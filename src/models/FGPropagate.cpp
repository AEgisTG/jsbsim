  ...
  if (debug_lvl & 16) { // Sanity checking
    if (from == 2) { // State sanity checking
      if (fabs(VState.vPQR.Magnitude()) > 1000.0) {
        ...
        error << endl << "Vehicle rotation rate is excessive (>1000 rad/sec): " << VState.vPQR.Magnitude() << endl;
        throw std::runtime_error(error.str());
      }
      if (fabs(VState.vUVW.Magnitude()) > 1.0e10) {
          std::stringstream error;
        error << endl << "Vehicle velocity is excessive (>1e10 ft/sec): " << VState.vUVW.Magnitude() << endl;
        throw std::runtime_error(error.str());
      }
      if (fabs(GetDistanceAGL()) > 1e10) {
          std::stringstream error;
        error << endl << "Vehicle altitude is excessive (>1e10 ft): " << GetDistanceAGL() << endl;
        throw std::runtime_error(error.str());
      }
    }
  }
